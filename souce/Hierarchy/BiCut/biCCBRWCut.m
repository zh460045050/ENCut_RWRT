function [ d, son_idx, ncut ] = biCCBRWCut( img, times, root_labels, lab)
...
% temp_lab = labels;
% 
% temp_lab(temp_lab < 0) = 0;
% temp_lab(temp_lab ~= 0) = 1;
% 
% imBw = temp_lab;
% imBw = bwmorph(imBw,'close');  %开运算
% imLabel = bwlabel(imBw) + 1;                %对各连通域进行标记
% imLabel = reshape(imLabel, [N, 1]);
% stats = regionprops(imLabel,'Area');    %求各连通域的大小
% area = cat(1,stats.Area);
% [~, a_idx] = max(area);
% labels((imLabel == a_idx)) = lab;
