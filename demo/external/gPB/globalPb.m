function [gPb_orient, gPb_thin, textons] = globalPb(img, times, rsz)
% syntax:
...
% December 2010
if nargin<3, rsz = 1.0; end     %narginä¸ºâ?number of input arguments???ç¼©å?????atlabä¸??ä¹??ä¸???°æ?ï¼??¨å??°ä????ï¼?nargin????¥å?????¥å???¸ª?°ç??½æ???if nargin<2, outFile = ''; end
...
outFile2 = '';
pb = max(mPb, [], 3);
salmap = doPBSaliency( img, pb );
[sPb] = spectralPb(mPb_rsz, times, salmap, orig_sz, outFile2);  %spectral cluster
delete(outFile2);
